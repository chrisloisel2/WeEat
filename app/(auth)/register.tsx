...
const RegisterScreen = () => {
	...
	const onSubmit = async (data: { password: any; confirmPassword: any; }) => {
		if (data.password !== data.confirmPassword) {
			return Toast.show({ type: 'error', text1: 'Les mots de passe ne correspondent pas' });
		}
		...
		try {
			...
			Toast.show({ type: 'error', text1: 'Erreur', text2: (err as any).message });
		}
	};

	return (
		<View style={tw`flex-1 bg-gray-900 justify-center px-6`}>
			<Text style={tw`text-white text-3xl font-bold text-center mb-6`}>Inscription</Text>
			...
			<View style={tw`mb-4`}>
				...
				<Controller
					control={control}
					name="password"
					rules={{ required: "Mot de passe requis", minLength: { value: 6, message: "Min 6 caractères" } }}
					render={({ field: { onChange, value } }) => (
						<TextInput
							style={tw`bg-gray-800 text-white px-4 py-3 rounded-lg`}
			...
			<View style={tw`mb-4`}>
				...
				/>
				{errors.confirmPassword && <Text style={tw`text-red-400 mt-1`}>{errors.confirmPassword.message}</Text>}
			</View>

			<TouchableOpacity onPress={handleSubmit(onSubmit)} style={tw`bg-blue-600 py-3 rounded-lg flex-row justify-center items-center`}>
				{isLoading ? <ActivityIndicator color="#fff" /> : <Text style={tw`text-white text-lg font-bold`}>S'inscrire</Text>}
			</TouchableOpacity>

			<TouchableOpacity onPress={() => navigation.navigate('login')} style={tw`mt-4`}>
				<Text style={tw`text-blue-400 text-center`}>Déjà inscrit ? Se connecter</Text>
			</TouchableOpacity>
